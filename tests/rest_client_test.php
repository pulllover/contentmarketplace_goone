...
class contentmarketplace_goone_rest_client_test extends \core_phpunit\testcase {
    ...
    public function test_401() {
        ...
        $this->expectExceptionMessage("Received 401 response when calling GO1 API (Called URL https://api.host/endpoint/function)");
        $client = new rest_client("https://api.host/endpoint", $curl);
        $client->get("function");
    }
    ...
    public function test_default_timeout_applies_when_not_overridden() {
        $curl = $this->createMock(\curl::class);
        $curl->method('get')->willReturnCallback(function ($url, $params, $options) {
            $this->assertSame(20, $options['TIMEOUT']);
            $this->assertSame(0, $options['CONNECTTIMEOUT']);
            return '{"test": "example"}';
        });
        $curl->method('get_info')->willReturn([
            'url' => 'https://api.host/endpoint/function',
    ...
    public function test_caller_supplied_timeout_is_respected() {
        $curl = $this->createMock(\curl::class);
        $curl->method('get')->willReturnCallback(function ($url, $params, $options) {
        ...
        $client = new rest_client("https://api.host/endpoint", $curl);
        $client->get("function", [], [], ['TIMEOUT' => 300]);
    }

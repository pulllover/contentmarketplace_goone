...
final class setup_wizard_default_controller extends \totara_form\form_controller {
    ...
    public function get_ajax_form_instance($idsuffix) {
        ...
        $free = $api->get_learning_objects_collection_count('free');
        $subscribe = $api->get_learning_objects_collection_count('subscribe');
        $custom = $api->get_learning_objects_collection_count('custom');
        $pay_per_seat = null;
        ...
        $parameters = [
            'courses_free' => (!empty($free)) ? local::format_integer($free) : get_string('notapplicable', 'contentmarketplace_goone'),
            'courses_subscribe' => (!empty($subscribe)) ? local::format_integer($subscribe) : get_string('notapplicable', 'contentmarketplace_goone'),
            'courses_custom' => (!empty($custom)) ? local::format_integer($custom) : get_string('notapplicable', 'contentmarketplace_goone'),
        ];

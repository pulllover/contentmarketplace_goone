...
class rest_client {
    ...
    public function patch(string $resourcename, array $params = [], array $headers = [], array $options = []) {
        ...
        list($url, $options) = $this->prepare_request($resourcename, [], $headers, $options);
        $response = $this->curl->patch($url, json_encode($params), $options);
        return $this->parse_response($response);
    }
    ...
    private function prepare_request(string $resourcename, array $params, array $headers, array $options) {
        ...
        // Timeouts are defaults only: callers may pass their own (e.g. large binary downloads).
    ...
    private function parse_response($response) {
        ...
        if (in_array($info['http_code'], [200, 201])) {
            ...
            // Is a scope (permission) missing from Go1 client Developer app?
            $data = json_decode($response);
            if (json_last_error() == 0 && isset($data->message) && $data->message == 'Missing scopes') {
                throw new missing_scope_exception($url, $response);
            }
            throw new invalid_token_exception($url, $response);
        } else {

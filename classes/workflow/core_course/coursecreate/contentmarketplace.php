...
class contentmarketplace extends marketplace_workflow {
    ...
    /**
     * With manual course creation enabled the workflow opens the Go1 content explorer, from which
     * the selected content is carried through to coursecreate.php. Otherwise it redirects to the
     * Go1 content curation page.
     *
     * @return \moodle_url
     */
    public function get_workflow_url(): \moodle_url {
        if (!helper::is_manual_course_creation_enabled()) {
            return new \moodle_url('/totara/contentmarketplace/contentmarketplaces/goone/curate.php');
        }
